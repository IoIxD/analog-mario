...
int SDL_RenderFillCircle(SDL_Renderer *renderer, int x, int y, int radius) {
  ...
  while (offsety >= offsetx) {
    ...
    status += SDL_RenderLine(renderer, x - offsety, y + offsetx, x + offsety,
                             y + offsetx);
    status += SDL_RenderLine(renderer, x - offsetx, y + offsety, x + offsetx,
                             y + offsety);
    status += SDL_RenderLine(renderer, x - offsetx, y - offsety, x + offsetx,
                             y - offsety);
    status += SDL_RenderLine(renderer, x - offsety, y - offsetx, x + offsety,
...
void to_pixel_format(const uint32_t *restrict in, uint32_t *restrict out,
  ...
  for (int i = 0; i < size; i++) {
    ...
    case SDL_PIXELFORMAT_ABGR8888: {
      out[i] = (in[i] & 0xff000000) | ((in[i] << 16) & 0x00ff0000) |
               (in[i] & 0x0000ff00) | ((in[i] >> 16) & 0x000000ff);
      break;
    }
    default:
