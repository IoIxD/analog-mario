...
void toggle_timer_resolution() {
  // toggles from high resolution to low to reduce CPU utilization during a
  // pause
  if (timerPeriod) {
    reset_resolution();
...
int adjusted_wait(Timer *timer) {
  Timer_t *t = timer->timer;
  int64_t req_period_ns =
      (int64_t)(t->period_ns - (t->diff.tv_sec * G + t->diff.tv_nsec));
  if (req_period_ns <= t->clock_res)
    return 0;

  struct timespec req = {.tv_sec = req_period_ns / G,
