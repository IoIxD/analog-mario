...
static void read_text_stream(char **list, const char *buf, size_t list_len,
                             size_t buf_len, size_t max_str_len);
...
void read_text_stream(char **list, const char *buf, size_t list_len,
                      size_t buf_len, size_t max_str_len) {
...
void load_auth_chunk(uint32_t len, Mapper *mapper, SDL_IOStream *file) {
  ...
  char *fields[4] = {nsf->song_name, nsf->artist, nsf->copyright, nsf->ripper};
...
void load_tlbl_chunk(uint32_t len, Mapper *mapper, SDL_IOStream *file) {
  ...
  read_text_stream(nsf->tlbls, (char *)chunk, nsf->total_songs, len,
                   MAX_TRACK_NAME_SIZE);
  free(chunk);
}
...
void init_song(Emulator *emulator, size_t song_number) {
  ...
  if (nsf->times != NULL) {
    nsf->tick_max =
        nsf->times[nsf->current_song == 0 ? 0 : nsf->current_song - 1];
    if (nsf->fade != NULL)
      nsf->tick_max +=
          nsf->fade[nsf->current_song == 0 ? 0 : nsf->current_song - 1];
  }
...
void next_song(Emulator *emulator, NSF *nsf) {
  nsf->current_song =
      nsf->current_song >= nsf->total_songs ? 1 : nsf->current_song + 1;
  init_song(emulator, nsf->current_song);
}

void prev_song(Emulator *emulator, NSF *nsf) {
  nsf->current_song =
      nsf->current_song <= 1 ? nsf->total_songs : nsf->current_song - 1;
  init_song(emulator, nsf->current_song);
}
...
void init_NSF_gfx(GraphicsContext *g_ctx, NSF *nsf) {
#ifdef __ANDROID__
  int offset_x = g_ctx->dest.x, offset_y = g_ctx->dest.y, width = g_ctx->dest.w,
      height = g_ctx->dest.h;
#else
  ...
  for (size_t i = 0; i < BAR_COUNT; i++) {
    bin_boundaries[i] =
        exp((log(20000) - log(20)) * i / (double)BAR_COUNT) * 20;
  }
  bin_boundaries[BAR_COUNT] = 20000;
  char buf[144] = {0};
  snprintf(buf, 144, "song: %s \nartist: %s \ncopyright: %s", nsf->song_name,
           nsf->artist, nsf->copyright);
  SDL_Color color = {192, 0x30, 0x0, 0xff};
  SDL_Surface *text_surf =
      TTF_RenderText_Solid_Wrapped(g_ctx->font, buf, 0, color, 0);
...
void render_NSF_graphics(Emulator *emulator, NSF *nsf) {
  ...
  double end = bin_boundaries[0], step = 20000.0f / AUDIO_BUFF_SIZE / 2,
         index = 0;
  size_t j = step / 20;
  for (size_t i = 0; i < BAR_COUNT; i++) {
    double total = 0;
  ...
  SDL_FRect dest;
  int max_bar_h = 0.4f * height, min_bar_h = 0.02f * height,
      bar_step = min_bar_h / 2;
  for (int i = 0; i < BAR_COUNT; i++) {
    ...
    amps[i] = amps[i] < min_bar_h   ? min_bar_h
              : amps[i] > max_bar_h ? max_bar_h
                                    : amps[i];
    dest.y = (height - amps[i]) / 2 + offset_y;
    dest.x = i * width / BAR_COUNT + offset_x;
    dest.w = width / BAR_COUNT - 1;
    dest.h = amps[i];
    SDL_SetRenderDrawColor(g_ctx->renderer, i * (g_ctx->width / BAR_COUNT), 0x0,
  ...
  if (song_num != nsf->current_song) {
    ...
    if (nsf->tlbls != NULL) {
      snprintf(str, 32 + MAX_TRACK_NAME_SIZE, "%d / %d: %s", nsf->current_song,
               nsf->total_songs, nsf->tlbls[current_song]);
    } else {
    ...
    if (nsf->times != NULL) {
      ...
      snprintf(str, 8, "%02d : %02d", nsf->tick_max / 60000,
               ((long)nsf->tick_max % 60000) / 1000);
      SDL_Color color1 = {0x0, 0x30, 192, 0xff};
      text_surf = TTF_RenderText_Solid(g_ctx->font, str, 0, color1);
      nsf->song_dur_max_tx =
          SDL_CreateTextureFromSurface(g_ctx->renderer, text_surf);
      ...
      nsf->song_num_rect.y =
          nsf->song_dur_max_rect.y - 0.12 * height - text_surf->h;
    } else {
  ...
  SDL_RenderTexture(g_ctx->renderer, nsf->song_info_tx, NULL,
                    &nsf->song_info_rect);
  SDL_RenderTexture(g_ctx->renderer, nsf->song_num_tx, NULL,
                    &nsf->song_num_rect);
  SDL_RenderTexture(g_ctx->renderer, nsf->song_dur_tx, NULL,
                    &nsf->song_dur_rect);
  SDL_RenderTexture(g_ctx->renderer, nsf->song_dur_max_tx, NULL,
                    &nsf->song_dur_max_rect);
#ifdef __ANDROID__
